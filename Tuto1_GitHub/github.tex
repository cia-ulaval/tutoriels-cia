...
Prenez le temps d'observer le contenu du dépôt. Vous y trouverez les dossiers associés aux quatre laboratoires de la session, ainsi que trois fichiers : \code{.gitignore}, \code{LICENSE} et \code{README.md}. Un bon dépôt devrait toujours contenir ces trois fichiers, ainsi qu'un fichier supplémentaire appelé \code{requirements.txt}. Nous y reviendrons plus tard, après avoir clôné le dépôt. \\
